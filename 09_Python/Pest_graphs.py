...
def time_series_graphs(prior=True, posterior=False, iter=1):
    if posterior:
...
def frequency_graphs(prior=True, posterior=False, iter=1, norm=False):
    ...
    plt.title(" frequency maximum total gal_flow")
...
def oneto1_graph(iter=None, ofilename="output_fname.pdf"):
...
if __name__ == '__main__':
    oneto1_graph(iter=1, ofilename=None)
    time_series_graphs(prior=True,posterior=True, iter=1)
    frequency_graphs(prior=True, posterior=True, iter=1, norm=True)
    frequency_graphs(prior=False, posterior=True, iter=1)
    frequency_graphs(prior=True, posterior=False, iter=1)
